javascript: (function () {
    ...
    function handleClick(event) {
        if (event.target.id === 'seleniumRecorderBtn' || event.target.id === 'seleniumRecorderDownload' || event.target.id === 'screenshotBtn' || !isRecording) {
            return;
        }
    ...
    function generateScript() {
        ...
        var code = [
            ...
            'from Script_Runner import navigate_to_url, find_and_perform_action, select_dropdown_by_visible_text, take_screenshot',
            'import time',
            '',
        ...
        for (var i = 0; i < steps.length; i++) {
            ...
            if (step.action === 'click') {
                ...
                code.push(`take_screenshot(driver, step_num=${stepNumber})`);
            }
        }
        code.push('');
